...
export function generateCoverages(productType) {
    const coverageMap = {
        auto: [
...
export function generatePolicies(customers, count) {
    const policies = [];
    const statusOptions = [
...
export function generateClaims(policies, count) {
    const claims = [];
...
export function generateLeads(count) {
    const leads = [];
    const producers = [
...
export function generateRenewals(policies) {
    const renewals = [];
    const renewalPolicies = policies.filter(p => p.status === 'renewal-pending' || p.status === 'active').slice(0, Math.ceil(policies.length * 0.15));
...
export function generateQuotes(leads, count) {
    const quotes = [];
